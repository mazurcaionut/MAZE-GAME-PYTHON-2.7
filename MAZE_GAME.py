...
def gata():
        tkMessageBox.showwarning("YOU LOST !","You touched the walls. The game is over.")
        f.destroy()
def coliziune():
        ...
        if 138 in L:
                gata()
        if 142 in L:
                tkMessageBox.showwarning("Keep looking!", "This is not the right square!")
        if 141 in L:
                tkMessageBox.showwarning("Congratulations !", "You won !")
        if 140 in L:
                tkMessageBox.showwarning("Keep looking!", "This is not the right square!")
...
reguliLbl=Label(cadru,text="MAZE TRAP",font=1)
reguliLbl.grid()

reguli1Lbl=Label(cadru,text="INSTRUCTIONS:",font=1)
reguli1Lbl.grid()

reguli2Lbl=Label(cadru,text="1)DON'T GO THROUGH THE WALLS.",font=1)
reguli2Lbl.grid()

reguli3Lbl=Label(cadru,text="2)IF THE FIRST RULE IS BROKEN THE GAME IS OVER.",font=1)
reguli3Lbl.grid()

reguli4Lbl=Label(cadru,text="3)ONLY ONE OF THE THREE SQUARES IS CORRECT.",font=1)
reguli4Lbl.grid()

buton = Button(cadru,text="START",command=desen)
buton.grid()
